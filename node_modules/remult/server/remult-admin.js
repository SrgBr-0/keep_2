...
function remultAdminHtml(options) {
    var rootPath = options.rootPath, head = options.head;
    return (0, get_remult_admin_html_js_1.getHtml)()
        .replace('<!--PLACE_HERE_HEAD-->', head)
        .replace('<!--PLACE_HERE_BODY-->', "<script>\n  window.optionsFromServer = ".concat(JSON.stringify({ rootPath: rootPath }), "\n</script>"));
}
            ...
            try {
                ...
                if (info) {
                    ...
                    if (info.type === 'reference' || info.type === 'toOne') {
                        ...
                        if (relRepo.metadata.apiReadAllowed) {
                            relations.push(tslib_1.__assign(tslib_1.__assign({}, relInfo), { where: where, entityKey: relRepo.metadata.key, key: x.key, caption: x.caption }));
                        }
                        return "continue";
                    }
                }
